...
The icon is the **RSO** mark: a neon-green power button (from the ORNATA logo)
framing the letters **R**azer **S**ynapse **O**rnata, on an anthracite squircle.
It is generated procedurally (no external assets, pure Pillow + the Avenir Next
system font) by [`assets/make_icon.py`](assets/make_icon.py). The matching ORNATA
wordmark logo (see [`assets/logo.png`](assets/logo.png)) is generated by
...
a monochrome power-button template baked into `src/main.js`.
